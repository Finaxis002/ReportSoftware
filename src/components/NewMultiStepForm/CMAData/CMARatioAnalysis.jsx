...
const storedGeneratedPdfDataString = localStorage.getItem("storedGeneratedPdfData");
const storedGeneratedPdfData = storedGeneratedPdfDataString 
  ? JSON.parse(storedGeneratedPdfDataString) 
  : {};

  // Destructure termLoanValues from the object.
  // If it's undefined, default to an empty array.
  const { termLoanValues = [] } = storedGeneratedPdfData.workingCapitalValues || {};
  const cumulativeLoanForPreviousYears =
    storedGeneratedPdfData.workingCapitalValues?.termLoanValues || [];
  ...
  const netWorth = Array.from({ length: projectionYears }).map((_, index) => {
    ...
    const reservesValue = Number(
      storedGeneratedPdfData?.receivedData?.cumulativeBalanceTransferred?.cumulativeBalanceTransferred?.[index] ||
        0
    );
  ...
  const totalDebtArray = Array.from({ length: projectionYears }).map(
    (_, index) => {
      const termLoan = Number(storedGeneratedPdfData?.marchClosingBalances?.[index + 1]) || 0;

      // ✅ Don't shift repayment index — use current year index
      const bankLoan =
        Number(
          storedGeneratedPdfData?.totalLiabilities?.repaymentValueswithin12months?.[index]
        ) || 0;
  ...
  const CurrentAssetsArray = Array.from({ length: projectionYears }).map(
    (_, index) => {
      const cashBalance =
        Number(storedGeneratedPdfData?.totalLiabilities?.closingCashBalanceArray?.[index]) || 0;
  ...
  const grossProfitSalesRatios = Array.from({ length: projectionYears }).map(
    (_, index) => {
      const sales = storedGeneratedPdfData?.totalRevenueReceipts[index] || 0; // Sales value
      const grossProfit = storedGeneratedPdfData?.computedData?.grossProfitValues[index] || 0; // Gross profit value
  ...
  const operatingProfitSalesRatios = Array.from({
    ...
    const sales = storedGeneratedPdfData?.totalRevenueReceipts[index] || 0;
    const operatingProfit = storedGeneratedPdfData?.computedData?.netProfitBeforeTax[index] || 0;
  ...
  const netProfitSalesRatio = Array.from({ length: projectionYears }).map(
    (_, index) => {
      const sales = storedGeneratedPdfData?.totalRevenueReceipts[index] || 0;
      const netProfit = storedGeneratedPdfData?.computedData?.netProfitAfterTax[index] || 0;
  ...
  const netWorthTotalLiabilitiesRatio = Array.from({
    ...
    const totalLiabilities =
      Number(storedGeneratedPdfData?.totalLiabilities?.totalLiabilitiesArray?.[index]) || 0;
  ...
  const yearlycurrentLiabilities =
    storedGeneratedPdfData?.totalLiabilities?.yearlyTotalLiabilities;

  // ✅ Calculate Current Ratio for each year
  const currentRatio = CurrentAssetsArray.map((assets, index) => {
    const liabilities =
      Number(storedGeneratedPdfData?.totalLiabilities?.yearlyTotalLiabilities?.[index]) || 0;
  ...
  const returnOnInvestment = Array.from({ length: projectionYears }).map(
    (_, index) => {
      const Npat = Number(storedGeneratedPdfData?.computedData?.netProfitAfterTax?.[index]) || 0;
  ...
  const averageDebtServiceCoverageRatio = calculateAverage(
    storedGeneratedPdfData?.dscr?.DSCR?.filter((r) => !isNaN(parseFloat(r)) && r !== 0).map(
      (r) => parseFloat(r)
    ) || []
  );

  const hideFirstYear = storedGeneratedPdfData?.totalRevenueReceipts?.[0] <= 0;
...
//   useEffect(() => {
//     if (
//       storedGeneratedPdfData?.totalLiabilities?.closingCashBalanceArray?.length > 0 ||
//       storedGeneratedPdfData?.totalLiabilities?.currentLiabilities?.length > 0
//     ) {
              ...
              <View>
                ...
                <View style={styles.tableRow}>
                  ...
                  {labels.map((_, localIdx) => {
                    ...
                    return (
                      ...
                      >
                        {formatNumber(storedGeneratedPdfData?.totalRevenueReceipts[gIdx] || 0)}
                      </Text>
                    );
                  })}
                ...
                <View style={styles.tableRow}>
                  ...
                  {labels.map((_, localIdx) => {
                    const gIdx = globalIndex(localIdx);
                    if (shouldSkipCol(gIdx)) return null;
                    const profit = storedGeneratedPdfData?.computedData?.grossProfitValues?.[gIdx] ?? 0;
                    return (
                      <Text
                ...
                <View style={styles.tableRow}>
                  ...
                  {labels.map((_, localIdx) => {
                    const gIdx = globalIndex(localIdx);
                    if (shouldSkipCol(gIdx)) return null;
                    const npbt = storedGeneratedPdfData?.computedData?.netProfitBeforeTax?.[gIdx] ?? 0;
                    return (
                      <Text
                        key={`npbt-${gIdx}`}
                        style={[
                ...
                <View style={styles.tableRow}>
                  ...
                  {labels.map((_, localIdx) => {
                    const gIdx = globalIndex(localIdx);
                    if (shouldSkipCol(gIdx)) return null;
                    const npat = storedGeneratedPdfData?.computedData?.netProfitAfterTax?.[gIdx] ?? 0;
                    return (
                      <Text
                        key={`npat-${gIdx}`}
                        style={[
                ...
                <View style={styles.tableRow}>
                  ...
                  {labels.map((_, localIdx) => {
                    ...
                    if (shouldSkipCol(gIdx)) return null;
                    const total =
                      (storedGeneratedPdfData?.totalLiabilities?.totalLiabilitiesArray || [])[
                        gIdx
                      ] ?? 0;
                    return (
                      <Text
                ...
                <View style={styles.tableRow}>
                  ...
                  {labels.map((_, localIdx) => {
                    const gIdx = globalIndex(localIdx);
                    if (shouldSkipCol(gIdx)) return null;
                    const total = storedGeneratedPdfData?.computedData?.cashProfitArray?.[gIdx] ?? 0;
                    return (
                      <Text
                        key={`cashprofit-${gIdx}`}
                        style={[
                ...
                >
                  ...
                  {labels.map((_, localIdx) => {
                    ...
                    const totalCashProfit = Array.isArray(storedGeneratedPdfData?.computedData?.cashProfitArray)
                      ? storedGeneratedPdfData?.computedData?.cashProfitArray.reduce(
                ...
                <View>
                  ...
                  <View style={styles.tableRow}>
                    ...
                    {labels.map((_, localIdx) => {
                      const gIdx = globalIndex(localIdx);
                      if (shouldSkipCol(gIdx)) return null;
                      const ratio = storedGeneratedPdfData?.dscr?.DSCR?.[gIdx] ?? 0;
                      return (
                        <Text
                          key={`dscr-${gIdx}`}
                          style={[
      ...
      <View>
        ...
        <View style={[styles.table, { borderRightWidth: 0 }]}>
          ...
          <View style={styles.tableHeader}>
            ...
            {financialYearLabels
              .slice(hideFirstYear ? 1 : 0) // ✅ Skip first year if receivedstoredGeneratedPdfData?.totalRevenueReceipts[0] < 0
          ...
          <View>
              ...
              {Array.from({ length: projectionYears }).map(
                (_, index) =>
                  (!hideFirstYear || index !== 0) && (
                    ...
                    >
                      {formatNumber(storedGeneratedPdfData?.totalRevenueReceipts[index] || 0)}
                    </Text>
                  )
              )}
            ...
            <View style={styles.tableRow}>
              ...
              {storedGeneratedPdfData?.computedData?.grossProfitValues.map(
                (profit, yearIndex) =>
            ...
            <View style={styles.tableRow}>
              ...
              {storedGeneratedPdfData?.computedData?.netProfitBeforeTax.map(
                (npbt, yearIndex) =>
                  (!hideFirstYear || yearIndex !== 0) && (
                    <Text
                      key={`npbt-${yearIndex}`}
            ...
            <View style={styles.tableRow}>
              ...
              {storedGeneratedPdfData?.computedData?.netProfitAfterTax.map(
                (npat, yearIndex) =>
                  (!hideFirstYear || yearIndex !== 0) && (
                    <Text
                      key={`npat-${yearIndex}`}
            ...
            <View style={styles.tableRow}>
              ...
              {Array.isArray(storedGeneratedPdfData?.totalLiabilities.totalLiabilitiesArray) &&
                storedGeneratedPdfData?.totalLiabilities.totalLiabilitiesArray.map(
                  (total, index) =>
                    (!hideFirstYear || index !== 0) && (
                      <Text
                        key={index}
            ...
            <View style={styles.tableRow}>
              ...
              {(storedGeneratedPdfData?.totalLiabilities?.yearlyTotalLiabilities ?? []).map(
                (total, index) =>
                  (!hideFirstYear || index !== 0) && (
                    <Text
                      key={index}
            ...
            <View style={styles.tableRow}>
              ...
              {/* ✅ Access the nested array correctly */}
              {Array.isArray(storedGeneratedPdfData?.computedData?.cashProfitArray) &&
                storedGeneratedPdfData?.computedData?.cashProfitArray.map(
                  (total, index) =>
                    (!hideFirstYear || index !== 0) && (
                      <Text
                        key={index}
            ...
            <View>
              ...
              <View style={styles.tableRow}>
                ...
                {Array.from({ length: projectionYears }).map((_, index) => {
                  ...
                  const ratio = storedGeneratedPdfData?.dscr?.DSCR?.[index] ?? 0;

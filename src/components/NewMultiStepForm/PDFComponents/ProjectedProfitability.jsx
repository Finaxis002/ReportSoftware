...
const adjustedRevenueValues = Array.from({
  ...
  console.log("adjusted revenue values",adjustedRevenueValues )
...
console.log( "gross Profit Values", grossProfitValues)
  ...
  const renderedIndirectExpenses = directExpense.filter((expense) => {
    ...
    const isAllYearsZero = Array.from({
      ...
      const adjustedYearIndex = hideFirstYear ? yearIndex + 1 : yearIndex;
      const escalated = calculateExpense(Number(expense.total) || 0, adjustedYearIndex);
      return escalated === 0;
    });
  ...
  expenseValue = calculateExpense(Number(expense.total) || 0, adjustedYearIndex);
          ...
          <View>
            ...
            {!isWorkingCapitalInterestZero && (<View style={[styles.tableRow, styles.totalRow]}>
              <Text style={stylesCOP.serialNoCellDetail}> {isInterestOnTermLoanZero ? 1 : 2}</Text>

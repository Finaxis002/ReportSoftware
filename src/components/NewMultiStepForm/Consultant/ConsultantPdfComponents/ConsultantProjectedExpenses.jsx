...
const ConsultantProjectedExpenses = ({
  formData,
  ...
  const RowWrapper = ({ children, index, isAlternating = true }) => {
  // If alternating is disabled, just render children
  if (!isAlternating) {
    return <View>{children}</View>;
  }
  
  // Apply alternate background (starting from index 0)
  const backgroundColor = index % 2 === 0 ? "transparent" : "#e2e2e2ff";
...
export default React.memo(ConsultantProjectedExpenses);

...
const ConsultantBreakEvenPoint = ({
  formData,
  ...
  const totalVariableExpenses = Array.from({
    ...
    const totalFromExpenses = allExpenses.reduce((total, expense) => {
      const expenseValue = getProjectedExpenseValue(expense, adjustedYearIndex);
    ...
    // Check if "Equity" is selected and include IOTL & IOWC in variable expenses
    ...
    const interestExpenseOnWorkingCapital =
      debtEquityOption === "Equity"
        ? calculateInterestOnWorkingCapital(adjustedYearIndex)
        : 0;
    ...
    return (
      ...
      totalAdvanceForYear +
      preliminaryExpense +
      interestOnTermLoan + // Include IOTL in Variable Expenses if Equity
      interestExpenseOnWorkingCapital // Include IOWC in Variable Expenses if Equity
    );
  });
  ...
  const totalFixedExpenses = Array.from({ length: projectionYears }).map(
    (_, yearIndex) => {
      ...
      // ✅ Extract Interest on Term Loan (but only if not "Equity")
      const interestOnTermLoan =
        debtEquityOption !== "Equity"
          ? parseFloat((yearlyInterestLiabilities[yearIndex] || 0).toFixed(2))
          : 0;

      // ✅ Extract Interest on Working Capital (but only if not "Equity")
      const interestExpenseOnWorkingCapital =
        debtEquityOption !== "Equity"
    ...
    return splitFinancialYearLabels.map((labels, pageIdx) => {
      ...
      const visibleAllExpenses = allExpenses.filter((expense) => {
        ...
        const isAllZeroOnPage = visibleLocalCols.every((localIdx) => {
          const gIdx = globalIndex(localIdx);
          return Number(getExpenseBaseValue(expense, gIdx)) === 0;
        });
            ...
            <View style={[styles.table]}>
              ...
              <View style={[styles.tableRow, styles.totalRow]}>
                ...
                {labels.map((_, localIdx) => {
                  const gIdx = globalIndex(localIdx);
                  if (shouldSkipCol(gIdx)) return null;
                  const v = formData?.MoreDetails?.ClosingStock?.[gIdx] ?? 0;
                  return (
                    <Text
                      key={`closing-${gIdx}`}
                      style={[
              ...
              <View style={[styles.tableRow, styles.totalRow]}>
                ...
                {labels.map((_, localIdx) => {
                  const gIdx = globalIndex(localIdx);
                  if (shouldSkipCol(gIdx)) return null;
                  const v = formData?.MoreDetails?.OpeningStock?.[gIdx] ?? 0;
                  return (
                    <Text
                      key={`opening-${gIdx}`}
                      style={[
              ...
              <View>
                ...
                {debtEquityOption === "Equity" && !isInterestOnTermLoanZero && (
                  <View style={[styles.tableRow, styles.totalRow]}>
                    {/* Serial Number */}
                    <Text
                      style={[
      ...
      <View style={[styleExpenses.paddingx, { paddingBottom: "30px" }]}>
        ...
        <View style={[styles.table]}>
          ...
          <View>
            ...
            {/* Interest On Working Capital */}
            {debtEquityOption === "Equity" && !isWorkingCapitalInterestZero && (
              <View style={[styles.tableRow, styles.totalRow]}>
                {/* Serial Number */}
                <Text
                  style={[
...
export default React.memo(ConsultantBreakEvenPoint);

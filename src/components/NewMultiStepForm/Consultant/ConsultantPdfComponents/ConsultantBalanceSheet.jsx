...
const ConsultantBalanceSheet = ({
    ...
    // Check if Depreciation is zero for all years - use computed data if available
    const isDepreciationZero = Array.from({ length: projectionYears }).every(
        (_, index) =>
            (formData?.computedData?.totalDepreciation?.[index] ?? totalDepreciationPerYear[index]) === 0
    );
                    ...
                    <View style={[styleExpenses.paddingx]}>
                        ...
                        <View style={[styles.table]}>
                            ...
                            <View style={styles.tableHeader}>
                                ...
                                {/* Generate Dynamic Year Headers for this page */}
                            ...
                            <View>
                                ...
                                <View style={styles.tableRow}>
                                    ...
                                    {labels.map((_, localIdx) => {
                                        ...
                                        const amount =
                                            receivedCummulativeTansferedData
                                                ?.cumulativeBalanceTransferred?.[gIdx] ?? 0;
                                ...
                                {!isBankTermLoanZero && (
                                    <View style={styles.tableRow}>
                                        ...
                                        {labels.map((_, localIdx) => {
                                            ...
                                            const marchBalance =
                                                receivedMarchClosingBalances?.[gIdx] || 0;
                                            const repaymentValue =
                                                repaymentValueswithin12months?.[gIdx] || 0;
                                ...
                                {!isWorkingCapitalLoanZero && (
                                    <View style={styles.tableRow}>
                                        ...
                                        {labels.map((_, localIdx) => {
                                            ...
                                            const loan = cumulativeLoanForPreviousYears[gIdx] || 0;
                                            return (
                                                <Text
                                                    key={gIdx}
                                                    style={[
                                    ...
                                    .map((liabilities, idx) => {
                                        return (
                                            <View style={styles.tableRow} key={idx}>
                                                ...
                                                {/* Cumulative values up to global year index */}
                                                {labels.map((_, localIdx) => {
                                                    const gIdx = globalIndex(localIdx);

                                                    const cumulative = (liabilities.years || [])
                                                        .slice(0, gIdx + 1)
                                ...
                                >
                                    ...
                                    {labels.map((_, localIdx) => {
                                        ...
                                        const total = totalLiabilitiesArray[gIdx] || 0;
                                        return (
                                            <Text
                                                key={gIdx}
                                                style={[
                            ...
                            <View>
                                ...
                                {/* Fixed Assets - Use computed data if available */}
                                {(formData?.computedData?.grossFixedAssetsPerYear?.some(val => val > 0) || computedFixedAssets.some(val => val > 0)) && (
                                    <View style={styles.tableRow}>
                                ...
                                {/* Show depreciation only if computed data has values OR if original data has values */}
                                ...
                                {!isNetFixedAssetsZero && (
                                    <View style={[styles.tableRow, styles.totalRow]}>
                                        ...
                                        {labels.map((_, localIdx) => {
                                            ...
                                            const value = computedNetFixedAssets[gIdx] || 0;
                                            return (
                                                <Text
                                                    key={gIdx}
                                                    style={[
                                ...
                                <View style={styles.tableRow}>
                                    ...
                                    {labels.map((_, localIdx) => {
                                        ...
                                        const balance = closingCashBalanceArray[gIdx] || 0;
                                        return (
                                            <Text
                                                key={gIdx}
                                                style={[
                                ...
                                {formData?.MoreDetails?.currentAssets
                                    ...
                                    .map((assets, index) => {
                                        const serialNumber = isInventoryZero
                                            ? index + 5
                                            : index + 6;
                                        ...
                                        return (
                                            <View style={styles.tableRow} key={index}>
                                                ...
                                                {/* Cumulative up to global index per visible year */}
                                                {labels.map((_, localIdx) => {
                                                    const gIdx = globalIndex(localIdx);
                                ...
                                {!isPreliminaryWriteOffAllZero && (
                                    <View style={[styles.tableRow, styles.totalRow]}>
                                        ...
                                        {labels.map((_, localIdx) => {
                                            ...
                                            const value = preliminaryExpenseBalanceSheet[gIdx] || 0;
                                            return (
                                                <Text
                                                    key={gIdx}
                                                    style={[
                                ...
                                >
                                    ...
                                    {labels.map((_, localIdx) => {
                                        ...
                                        const total = totalAssetArray[gIdx] || 0;
                                        return (
                                            <Text
                                                key={gIdx}
                                                style={[
                        ...
                        {(formData?.computedData?.totalDepreciation?.some(val => val > 0) || totalDepreciationPerYear.some(val => val > 0)) && (
                            ...
                            >
                                ...
                                {Array.from({ length: projectionYears }).map((_, yearIndex) => (
                                    ...
                                    >
                                        {formatNumber(
                                            formData?.computedData?.totalDepreciation?.[yearIndex] ??
                                            totalDepreciationPerYear[yearIndex] ??
                                            0
                                        )}
                                    </Text>
                                ))}
...
export default React.memo(ConsultantBalanceSheet);

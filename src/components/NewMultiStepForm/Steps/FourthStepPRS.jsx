...
const FourthStepPRS = ({ formData, onFormDataChange, onProjectionYearChange }) => {
  ...
  useEffect(() => {
    if (formData?.ProjestReportSetting) {
      const newData = {
        ...localData,
        ...formData.ProjestReportSetting,
      };

      // Prevent unnecessary updates
      if (!prevDataRef.current || JSON.stringify(prevDataRef.current) !== JSON.stringify(newData)) {
        console.log("Populating ProjestReportSetting data:", newData);
  ...
  const handleChange = (e) => {
    ...
    if (name === "ProjectionYears") {
      onProjectionYearChange(value);  // If you want to notify the parent
      console.log("Set projection year:", value); // Corrected the log
    }
  };
